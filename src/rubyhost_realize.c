...
static int body_for_limb(const td_limb* l, float x, float y, float theta,
...
static bool toy_realize(VALUE toyv) {
    ...
    for (long i = 0; i < RARRAY(limbs)->len; i++) {
        ...
        if (ln->body < 0 && ln->ldef) {
            ln->body = body_for_limb(ln->ldef, (float)ln->px, (float)ln->py,
                                     (float)ln->orient, S, t->instance_id);
            if (ln->body < 0) {
